...
const getMyApplications = async (email, limit, page, search, status) => {
...
const updateProfileInfo = async (email, body) => {
...
const userOverView = async (email) => {
    const user = await prisma.user.findUnique({
        where: { email },
    });
    if (!user) {
...
const recruiterOverView = async (email) => {
    const user = await prisma.user.findUnique({
        where: { email },
    });
    if (!user) {
...
const allUsers = async (limit, page) => {
